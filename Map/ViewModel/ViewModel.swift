...
class ViewModel:ObservableObject {
    let constData = 2000
    @Published var cities:Dictionary<String,City> =
    ...
    init(){
        getJsonData(date: Date())
    }
    
    func getJsonData(date:Date){
                ...
                DispatchQueue.main.async { [weak self] in
                    if covidData.itemList.count == 0{
                        if let cities = self?.cities{
                            for city in cities{
                                self?.cities[city.key]?.amount = 0
                            }
                        }
                    }
                    for data in covidData.itemList{
                        self?.cities[data.name_jp]?.amount = Int(data.npatients) ?? 0
                    }
                }
